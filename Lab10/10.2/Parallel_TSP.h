...
int Ncities=0, terr_type, pop_size, Ngen, Nmigr, Ngen_settled;               //terr_type = 0 -> cities randomly on a circumference, terr_type !=0 -> cities randomly in a square
...
class territory {
  ...
  territory(const string& filename) {
    double lon, lat;
    ...
    while (CityFile >> lon >> lat) {
      m_Territory.push_back({lon, lat});
      Ncities++;
    }
