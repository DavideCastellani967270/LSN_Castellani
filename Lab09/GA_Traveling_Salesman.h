...
int Ncities, terr_type, pop_size, Ngen;               //terr_type = 0 -> cities randomly on a circumference, terr_type !=0 -> cities randomly in a square
  ...
  territory( int terr_type ){
    ...
    if(terr_type == 0){                       //circumference
      for (int i = 0; i < Ncities; i++){
        ...
        angle = rnd.Rannyu(0, 2*M_PI);                          //generates a random angle in radiants
        m_Territory.push_back( {cos(angle), sin(angle)} );      //computes the coordinates of the city
      }
    } else {                                    //square
      for (int i = 0; i < Ncities; i++){
          
        m_Territory.push_back( {rnd.Rannyu(), rnd.Rannyu()} );  //generates the xy coordinates by drawing two uniform numbers between [0,1)
      }
    }
  }
...
class population : public chromosome {
  ...
  //methods
  //this method returns the chromosome in th position pos of the population
  ...
  void NewGen(){
    ...
    for (int i = int(pop_size/4); i<pop_size-1; i=i+2){     //cycles over pairs of the least fitting chromosomes
      r_cross = rnd.Rannyu();
