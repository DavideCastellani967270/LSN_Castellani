...
number of blocks. Here, it is also introduced the computation of the radial density function g(r) and added the tail corrections for U/N and P, crucial in the
predictions of quantities that used the Metropolis algorithm.
...
void Input(void){
  ...
  bin_size = (box/2.) / nbins;                            //computes bin size for the calculation of g(r)
  ...
  ig = 5;      //array position referring to g(r)
  ...
  cout << "System has successfully reached equilibrium." << endl<<endl;
...
void Measure() {
  ...
  for (int i=0; i<npart-1; ++i){          //cycles over every possible pair of particles
    for (int j=i+1; j<npart; ++j){ 
      ...
      if ( dr < box/2. ){                       //the only distances accounted are the ones smaller than half the box side lenght
        g[ int (dr/bin_size) ] += 2;            //the update is +2 because we're cyclyng over unique couples, so it's necessary to account for both the particles
      }
...
void Reset(int iblk) {
  ...
  for(int i=0; i<nbins; i++){
    g[i] =0;                          //resets all the counts of the block
  }
...
void Averages(int iblk) {
  ...
  if(iNVET){
    Epot.open("../MC.epot.out",ios::app);
    Press.open("../MC.press.out",ios::app);
    RadDis.open("../MC.radial_dis.out",ios::app);
  } else{
    Epot.open("../MD.epot.out",ios::app);
    Press.open("../MD.press.out",ios::app);
    RadDis.open("../MD.radial_dis.out",ios::app);
  }
  ...
  */
  ...
  //g(r)
  for(int i=0; i<nbins; i++){
    rI = i * bin_size;              //computes the lower limit r of the ith bin
    rII = rI + bin_size;            //computes the upper limit r+dr of the ith bin

    //normalizes the counts of the ith bin by the step number (so that we get a probability distribution) and by rho*Npart* DeltaV of the bin
    stima_gi = (g[i]/blk_norm) *3./(4.*M_PI*rho*npart * (pow(rII, 3) - pow(rI, 3)));

    glob_av_g[i] += stima_gi;                                  //sum of the g(r) for all bin [r,r+dr) -> to get the global average
    glob_av2_g[i] += stima_gi*stima_gi;
    err_gi = Error(glob_av_g[i],glob_av2_g[i],iblk);     //computes the statistical error
  
    //prints the block number, last estimate of the g(r) in the ith bin [r,r+dr), its progressive global average and statistical error
